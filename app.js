const CONFIG = {
    urlSheetsActrices: "https://docs.google.com/spreadsheets/d/e/2PACX-1vTXoEu4urfaWYbqgIpqh9YA9ei7pThMVLNpq9AEVYh7BchIUyYokM_9eggj7nEVNCweoz3NaMJYbYM8/pub?gid=0&single=true&output=csv",
    urlSheetsVideos: "https://docs.google.com/spreadsheets/d/e/2PACX-1vTXoEu4urfaWYbqgIpqh9YA9ei7pThMVLNpq9AEVYh7BchIUyYokM_9eggj7nEVNCweoz3NaMJYbYM8/pub?gid=1597144864&single=true&output=csv"
};
...
function aplicarConfiguracion() {
    ...
    setVar('--config-color-texto-btn-cen-iluminado', 'COLOR_TEXTO_BTN_CEN_ILUMINADO', '#030712');
    
    const colorBordeCen = setVar('--config-color-borde-conjunto-btn-cen', 'COLOR_BORDE_CONJUNTO_BTN_CEN', '#ca8a04');
    root.style.setProperty('--config-color-borde-conjunto-btn-cen-alpha', hexToRgba(colorBordeCen, 0.3));
    ...
    setVar('--config-color-texto-pestanas-home-iluminadas', 'COLOR_TEXTO_PESTANAS_HOME_ILUMINADAS', '#030712');
    ...
    setVar('--config-color-texto-pestanas-actriz-iluminadas', 'COLOR_TEXTO_PESTANAS_ACTRIZ_ILUMINADAS', '#030712');
    ...
    setVar('--config-color-texto-opcion-ordenar-por', 'COLOR_TEXTO_OPCION_ORDENAR_POR', '#eab308');
    setVar('--config-color-borde-buscador', 'COLOR_BORDE_BUSCADOR', '#ca8a04');
    
    setVar('--config-color-spinner-carga', 'COLOR_SPINNER_CARGA', '#eab308');
    setVar('--config-color-texto-error-conexion', 'COLOR_TEXTO_ERROR_CONEXION', '#ef4444');

    setVar('--config-color-titulo-modal-nombres', 'COLOR_TITULO_MODAL_NOMBRES', '#eab308');
    setVar('--config-color-titulo-modal-ayuda', 'COLOR_TITULO_MODAL_AYUDA', '#eab308');
    ...
    try {
        contenedorPrincipal.innerHTML = `
            <div class="flex flex-col items-center justify-center py-24 text-center gap-3">
                <div class="w-8 h-8 border-4 border-t-transparent rounded-full animate-spin dinamico-spinner-carga"></div>
        ...
        const [resActrices, resVideos] = await Promise.all([
            fetch(CONFIG.urlSheetsActrices).then(r => {
                if (!r.ok) throw new Error("Error al obtener actrices");
                return r.text();
            }),
            fetch(CONFIG.urlSheetsVideos).then(r => {
                if (!r.ok) throw new Error("Error al obtener videos");
                return r.text();
            })
        ]);
        ...
        contenedorPrincipal.innerHTML = `
            <div class="text-center py-20 px-4">
                <p class="font-black text-xs dinamico-texto-error-conexion">❌ ERROR DE CONEXIÓN</p>
            </div>
        `;
    }
}
...
function generarListaActricesUnicas() {
    ...
    BD_ACTRICES.forEach(a => {
        if (a.nombreactriz) {
            const key = a.nombreactriz.trim().toLowerCase();
            mapaActrices[key] = {
                codigoActriz: a.codigoactriz ? a.codigoactriz.trim() : "",
    ...
    LISTA_ACTRICES_UNICAS = Array.from(setActrices).map(nombre => {
        ...
        const info = mapaActrices[key] || {};
        return {
            actriz: nombre,
            codigoActriz: info.codigoActriz || "",
            nombreVisible: obtenerNombreVisibleActriz(nombre),
            ultimaactualizacion: info.ultimaactualizacion || "---"
        };
    });
...
function configurarEventos() {
    document.querySelectorAll(".tab-item").forEach(boton => {
        boton.addEventListener("click", (e) => {
            ...
            if (tabTarget === pestañaActiva && pestañaActiva !== "ayuda") {
                const hayBusquedaActiva = inputBuscar.value.trim() !== "";
                const hayPaginacion = paginaActual > 1;
                const hayFiltroOrden = criterioOrden !== "reciente";
                const hayFiltroProtagonismo = filtroProtagonismo !== "todos";
            
                if (scrollActual <= 10 && !hayBusquedaActiva && !hayPaginacion && !hayFiltroOrden && !hayFiltroProtagonismo) {
                    return;
                }
    ...
    btnCatScen.addEventListener("click", manejarClickInicio);
...
function actualizarUIHeadernavigation() {
    ...
    if (pestañaActiva === "actriz_individual") {
        ...
        if (btnVolverActrices) {
            btnVolverActrices.className = "px-4 py-2 rounded-full text-sm font-bold bg-gray-800 text-gray-300 hover:text-white transition-colors border border-gray-700 active:scale-95 flex items-center justify-center";
        }
        if (nombreActrizTitulo) {
            nombreActrizTitulo.textContent = obtenerNombreVisibleActriz(actrizSeleccionada);
        }

        const regActrizCab = BD_ACTRICES.find(a => a.nombreactriz && a.nombreactriz.toLowerCase().trim() === actrizSeleccionada.toLowerCase().trim());
        const idActrizCab = regActrizCab && regActrizCab.codigoactriz ? regActrizCab.codigoactriz.trim() : "";
        
        const imgCabecera = document.getElementById("imagen-actriz-cabecera");
        if (imgCabecera) {
            if (idActrizCab) {
                imgCabecera.src = `portadas/act/${idActrizCab}.jpg`;
                imgCabecera.onerror = () => {
                    imgCabecera.src = `portadas/act/${idActrizCab}.webp`;
                    imgCabecera.onerror = () => {
                        imgCabecera.src = `portadas/act/${idActrizCab}.png`;
        ...
        document.querySelectorAll(".tab-item").forEach(b => {
            ...
            if (!tabAttr && pestañaActiva === "todos") {
                ...
                b.className = "tab-item px-4 py-2 rounded-full text-sm font-bold bg-gray-800 text-gray-300 hover:text-white transition-colors border border-gray-700 active:scale-95 flex items-center justify-center";
            } else {
                b.className = "tab-item px-5 py-2 rounded-full text-sm font-bold bg-gray-800 text-gray-300 hover:text-white transition-colors border border-gray-700 active:scale-95";
            }
        });
    }
}
            ...
            <div class="portada-contenedor block w-full group relative cursor-pointer">
                <div class="relative bg-gray-950 aspect-[1.5/1] w-full flex items-center justify-center overflow-hidden rounded-lg border border-gray-800">
                    <img src="portadas/vid/${codigoReal}.jpg" 
                         onerror="this.onerror=()=> { this.onerror=()=> { this.src='https://placehold.co/600x400/111827/4b5563?text=${codigoReal}'; }; this.src='portadas/vid/${codigoReal}.png'; }; this.src='portadas/vid/${codigoReal}.webp';" 
...
function renderizarListaActrices() {
    ...
    actricesPagina.forEach(actrizObj => {
        ...
        const idActriz = actrizObj.codigoActriz || "default";

        item.innerHTML = `
            <div class="w-full aspect-[1/1.3333] rounded-lg bg-gray-950 border border-gray-800 overflow-hidden shadow">
                <img src="portadas/act/${idActriz}.jpg" 
                     onerror="this.onerror=()=> { this.onerror=()=> { this.src='https://placehold.co/300x400/111827/ffffff?text=${actrizObj.nombreVisible.charAt(0)}'; }; this.src='portadas/act/${idActriz}.png'; }; this.src='portadas/act/${idActriz}.webp';" 
                     class="w-full h-full object-fill">
            </div>
